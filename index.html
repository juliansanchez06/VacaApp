...
function getSliderConfig(unit, step, value) {
  ...
  if (unit === "km")     return { min: 0, max: 500,     step: s };
  if (unit === "$/km")   return { min: 0, max: 2000,    step: s };
        ...
        {on && (
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <Field label={`KM ${tipo}`} value={gastos[kmKey]} onChange={set(kmKey)} unit="km" step={10} />
              <Field label="Precio / km" value={gastos[pkmKey]} onChange={set(pkmKey)} unit="$/km" step={10} />
            </div>
  ...
  const ComisionBlock = ({ tipo }) => {
    ...
    return (
      <div className={`rounded-xl border p-3 space-y-2 transition-all ${on ? "bg-white border-slate-200" : "bg-slate-50 border-slate-100"}`}>
        ...
        {on && (
          <Field label="Porcentaje" value={gastos[key]} onChange={set(key)} unit="%" step={0.5} />
        )}
      </div>
    );
  };
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {/* Desglose Financiero Feedlot — 4 componentes del margen */}
          <div className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-4 space-y-3">
            ...
            <div className="bg-white rounded-lg border border-indigo-200 p-3 space-y-1">
              ...
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-500">({fmtMoney(calc.b.ingresoBruto > 0 ? opB.precioVentaKg : 0, 0)} − {fmtMoney(calc.b.costoKgGanado, 0)}) × {fmtKg(calc.b.gpvTotal * base.cantidad)}</span>
            ...
            {/* 3 — Gastos operativos y comerciales */}
            <div className="bg-white rounded-lg border border-indigo-200 p-3 space-y-1">
              <div className="flex items-center gap-1.5 mb-1">
              ...
              {[
                ...
                <div key={i} className="flex justify-between text-xs text-slate-500">
                  <span>{r.label}</span>
                  <span className="font-mono text-red-500">{fmtMoney(r.value)}</span>
                </div>
              ))}
            </div>
    ...
    <div className="rounded-2xl border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50 shadow-sm mb-6">
      ...
      {open && (
        <div className="px-5 pb-5 space-y-5 border-t border-emerald-200/60 pt-4">
          ...
          {/* Gastos Comerciales dentro del panel */}
          <div>
...
export default function EstrategiaComercial() {
  ...
  // fletes = km * precio/km (calculado en el panel y propagado)
